            ...
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div id="m_url_category_type">
                            <label for="type">Category name for menu item</label>
                            <select class="form-control select2" tabindex="-1" aria-hidden="true" id="selected_category">
                                <optgroup label="Custom">
                                    <option selected="selected" value="">Select a category</option>
                                </optgroup>
                                <optgroup label="Categories With Sub">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->slug }}" data-type="category" data-title="{{ $category->name }}">{{ $category->name }}</option>
                                        @foreach($category->children as $subCategory)
                                            <option value="{{ $subCategory->slug }}" data-type="categories" data-title="{{ $subCategory->name }}">--{{ $subCategory->name }}</option>
                                        @endforeach
                                    @endforeach
                                </optgroup>
                                <optgroup label="Without Sub">
                                    @foreach($categoriesWithNoSubs as $category)
                                        <option value="{{ $category->slug }}" data-type="category" data-title="{{ $category->name }}">{{ $category->name }}</option>
                                    @endforeach
                                </optgroup>
                            </select>
                        ...
                        <label for="icon_class">{{ __('voyager::menu_builder.icon_class') }} <a
                                    ...
                                    target="_blank"></a>{!! __('voyager::menu_builder.icon_class2') !!}</label>
    ...
    <script>
        $(document).ready(function () {
            ...
            var $m_modal       = $('#menu_item_modal'),
                ...
                $m_url_category_type  = $('#m_url_category_type'),
                $m_route       = $('#m_route'),
            ...
            /**
             * Category auto complete after click
             */
            $('#selected_category').change(function(){
                let type = $('#selected_category option:selected').attr('data-type');
                let title = $('#selected_category option:selected').attr('data-title');
                let url = 'shop?' + type + '=' + $('#selected_category').val();
                $('#m_url').val(url);
                $('#m_title').val(title);
            });
        });
