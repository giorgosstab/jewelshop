    ...
    private function installThemes() {
        ...
        foreach($themes as $theme){
            if(isset($theme->folder)){
                $theme_exists = VoyagerTheme::where('folder', '=', $theme->folder)->first();
                ...
                if(!isset($theme_exists->id)){
                    ...
                    VoyagerTheme::create(['name' => $theme->name, 'folder' => $theme->folder, 'version' => $version]);
...
//            $this->deactivateThemes();
            ...
            return redirect()
                ->route("theme.index")
                ->with([
                    'message'    => "Successfully deactivate " . $theme->name . " theme.",
                    'alert-type' => 'success',
                ]);
        } else {
            return redirect()
    ...
    public function options_save(Request $request, $theme_folder){
        ...
        foreach($request->all() as $key => $content){
            ...
            if(!$this->stringEndsWith($key, '_details__theme_field') && !$this->stringEndsWith($key, '_type__theme_field') && $key != '_token'){
                ...
                $option = VoyagerThemeOption::where('voyager_theme_id', '=', $theme->id)->where('key', '=', $key)->first();
                ...
                if(isset($option->id)){
                    ...
                    VoyagerThemeOption::create(['voyager_theme_id' => $theme->id, 'key' => $key, 'value' => $value]);
                }
            }
        }
    ...
    private function deactivateThemes(){
        VoyagerTheme::query()->update(['active' => 0]);
    }
