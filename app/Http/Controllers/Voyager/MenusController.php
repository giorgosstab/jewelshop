...
class MenusController extends VoyagerMenuController
{
    public function builder($id)
    {
        ...
        $categories = CategoryJewel::where('status', 'like', 'PUBLISHED')->with('children')->whereHas('children', function($query){
            $query->where('status', 'like', 'PUBLISHED');
        })->get();
        $categoriesWithNoSubs = CategoryJewel::where('status', 'like', 'PUBLISHED')->where('parent_id',null)->with('children')->whereDoesntHave('children', function($query){
            $query->where('status', 'like', 'PUBLISHED');
        })->get();

        return Voyager::view('voyager::menus.builder', compact('menu', 'isModelTranslatable', 'categories', 'categoriesWithNoSubs'));
    }
